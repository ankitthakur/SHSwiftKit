#
# Be sure to run `pod lib lint SwiftKit.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see http://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = "SwiftKit"
  s.version          = "0.1.0"
  s.summary          = "A short simple kit in Swift, which will share common utilities, wrappers and examples."

# This description is used to generate tags and improve search results.
#   * Think: What does it do? Why did you write it? What is the focus?
#   * Try to keep it short, snappy and to the point.
#   * Write the description between the DESC delimiters below.
#   * Finally, don't worry about the indent, CocoaPods strips it!  
s.description      = "# SwiftKit ![alt tag](https://github.com/ankitthakur/SwiftKit/blob/master/SwiftKit.png)
                            It is the simple kit in Swift, which will share common utilities and examples.

                            > Cocoapods Integration
                            ```pod 'SwiftKit'```

                            Tasks In Progress
                            * Basic HTTP Calls with NSURLSession
                            * Uploading Data on Server
                            * Downloading Data from Server
                            * Support Chunks and Progress of uploads/downloads in percentage format."


  s.homepage         = "https://github.com/ankitthakur/SwiftKit"
  # s.screenshots     = "www.example.com/screenshots_1", "www.example.com/screenshots_2"
  s.license          = 'MIT'
  s.author           = { "ankitthakur" => "ankitthakur85@gmail.com" }
  s.source           = { :git => "https://github.com/ankitthakur/SwiftKit.git", :tag => s.version.to_s }
  # s.social_media_url = 'https://twitter.com/ankitthakur85'

  s.platform     = :ios, '8.0'
  s.requires_arc = true

  s.source_files = 'Pod/Classes/**/*'
  s.resource_bundles = {
    'SwiftKit' => ['Pod/Assets/*.png']
  }

  # s.public_header_files = 'Pod/Classes/**/*.h'
  # s.frameworks = 'UIKit', 'MapKit'
  # s.dependency 'AFNetworking', '~> 2.3'
end
