...
public class SHDeviceCarrierInfo {
    ...
    public private(set) lazy var carrierDescription:String! = {
        let carrierdescription = self.hasCellularCarrier ? "\(self.cellularCarrierName) \(self.cellularConnectionType) \(self.cellularCarrierCountry)" : "\(self.cellularCarrierName) \(self.cellularCarrierCountry)";
