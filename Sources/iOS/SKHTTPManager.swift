...
//  Pods
//
//  Created by ankitthakur on 28/01/16.
//
//
...
typealias ProgressBlock = (data: NSData?, percent: Double?, error:NSError?) -> Void
...
enum MediaType {
    case Data(NSURL, [String:AnyObject]?) // request url, http body parameters, BLOB
    case BLOB(NSURL, [String:AnyObject]?, NSData?) // request url, http body parameters, BLOB
    case File(NSURL, [String:AnyObject]?, NSURL) // // request url, http body parameters, file url
    case Stream(NSURL, [String:AnyObject]?, NSInputStream) // request url, http body parameters, stream
}



class SKHTTPManager: NSObject, NSURLSessionDelegate, NSURLSessionTaskDelegate {
    
    static let sharedInstance = SKHTTPManager()
    ...
    var backgroundQueue:dispatch_queue_t = SKAsyncManager.sharedInstance.GlobalUtilityQueue;
